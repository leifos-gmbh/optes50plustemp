 ...
 * 
 * @version		$Id: class.ilObjTestGUI.php 60123 2015-07-23 12:04:43Z bheyser $
 *
...
class ilObjTestGUI extends ilObjectGUI
{
	...
	function executeCommand()
	{
		...
		switch($next_class)
		{
			...
			default:
				...
				$q_gui->object->setObjId($this->object->getId());
				if(!$_GET['sel_question_types']) $qType = assQuestion::getQuestionTypeFromDb($_GET['q_id']); else
				{
				...
				if($_REQUEST['test_express_mode']) $this->ctrl->setParameterByClass($qType."GUI", 'test_express_mode', 1);

				#global $___test_express_mode;
				#$___test_express_mode = true;
				if(!$q_gui->isSaveCommand()) $_GET['calling_test'] = $this->object->getRefId();
	...
	function addDidacticTemplateOptions(array &$a_options)
	{
		...
		if(count($defaults))
		{
			...
			{
				$a_options["tstdef_".$row["test_defaults_id"]] = array($row["name"], $this->lng->txt("tst_default_settings"));
			}
		}
		...
		if($templates)
		{
			...
			{
				$a_options["tsttpl_".$item["id"]] = array($item["title"], nl2br(trim($item["description"])));
			}
		}
	}
	...
	function applyFilterCriteria($in_rows)
	{
		...
		{
			$result = $ilDB->query('SELECT count(solution_id) count
				FROM tst_solutions
	...
	function getBrowseForQuestionsTab(&$tabs_gui)
	{
		...
		if($ilAccess->checkAccess("write", "", $this->ref_id))
		{
			...
			$tabs_gui->addTarget("tst_browse_for_questions", $this->ctrl->getLinkTarget($this, "browseForQuestions"), array("browseForQuestions", "filter", "resetFilter", "resetTextFilter", "insertQuestions"), "", "", TRUE);
		}
	}
	...
	function getRandomQuestionsTab(&$tabs_gui)
	{
		...
		if($ilAccess->checkAccess("write", "", $this->ref_id))
		{
			...
			$tabs_gui->addTarget("random_selection", $this->ctrl->getLinkTarget($this, "randomQuestions"), array("randomQuestions"), "", "");
		}
	}
	...
	function getQuestionsSubTabs()
	{
		...
		if(!in_array('questions', $template->getHiddenTabs()))
		{
			...
			if(in_array($a_cmd, array('questions', 'createQuestion')) || ($a_cmd == 'removeQuestions' && !$_REQUEST['test_express_mode'])) $this->tabs_gui->activateSubTab('edit_test_questions');
		}
		#}
	...
	function getStatisticsSubTabs()
	{
		...
		// user results subtab
		$ilTabs->addSubTabTarget("eval_all_users", $this->ctrl->getLinkTargetByClass("iltestevaluationgui", "outEvaluation"), array("outEvaluation", "detailedEvaluation", "exportEvaluation", "evalUserDetail", "passDetails", "outStatisticsResultsOverview", "statisticsPassDetails"), "");

		// aggregated results subtab
		$ilTabs->addSubTabTarget("tst_results_aggregated", $this->ctrl->getLinkTargetByClass("iltestevaluationgui", "eval_a"), array("eval_a"), "", "");

		// question export
		$ilTabs->addSubTabTarget("tst_single_results", $this->ctrl->getLinkTargetByClass("iltestevaluationgui", "singleResults"), array("singleResults"), "", "");
	}
	...
	function getSettingsSubTabs($hiddenTabs = array())
	{
		...
		// general subtab
		$ilTabs->addSubTabTarget('general', $this->ctrl->getLinkTargetByClass('ilObjTestSettingsGeneralGUI'), '',                                            // auto activation regardless from cmd
		...
		// scoring subtab
		$ilTabs->addSubTabTarget('scoring', $this->ctrl->getLinkTargetByClass('ilObjTestSettingsScoringResultsGUI'), '',                                             // auto activation regardless from cmd
	...
	function getTabs(&$tabs_gui)
	{
		...
		if(strcmp(strtolower(get_class($this->object)), "ilobjtest") == 0)
		{
			...
			if($ilAccess->checkAccess("write", "", $this->ref_id) && !in_array('assQuestions', $hidden_tabs))
			{
				...
				$tabs_gui->addTarget("assQuestions", //$this->ctrl->getLinkTarget($this,'questions'),
	...
	{
		if($this->create_question_mode) return;
	...
	public function copyQuestionObject()
	{
		...
		if($_REQUEST['q_id'] && !is_array($_REQUEST['q_id'])) $ids = array($_REQUEST['q_id']); else if($_REQUEST['q_id']) $ids = $_REQUEST['q_id']; else
		{
		...
		foreach($ids as $id)
		{
			...
			if($question)
			{
				...
				$i = 2;
				while(in_array($title.' ('.$i.')', $questionTitles)) $i++;
